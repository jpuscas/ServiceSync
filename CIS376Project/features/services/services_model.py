...
def create_service(cursor, service_name: str, service_type: str, service_date: str, service_time: str, leader_id: int, org_id: str = 'default'):
    ...
    cursor.execute('''
    INSERT INTO services (service_name, service_type, service_date, service_time, leader_id, org_id)
    VALUES (?, ?, ?, ?, ?, ?)
    ''', (service_name, service_type, service_date, service_time, leader_id, org_id))
...
def get_service_by_id(cursor, service_id: int, org_id: str = 'default'):
    ...
    WHERE s.service_id = ? AND s.org_id = ?
    ''', (service_id, org_id))
...
def list_services_for_user(cursor, user_id: int, org_id: str = 'default'):
    ...
    WHERE ((sm.user_id = ? AND (sm.accepted IS NULL OR sm.accepted = 1)) OR s.leader_id = ?) AND s.org_id = ?
