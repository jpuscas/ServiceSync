...
from features.users.users_model import get_user_by_id, get_user_by_username, user_belongs_to_org
...
def _validate_service_access(cursor, admin_id, service_id, org_id):
    ...
    if not admin_user or not user_belongs_to_org(admin_user, org_id):
...
def add_member_to_service(admin_id, service_id, member_username, role_name, org_id, member_user_id=None, cursor=None):
    ...
    try:
        ...
        if member_user_id is not None:
            ...
            if member and not user_belongs_to_org(member, org_id):
                member = None
        else:
